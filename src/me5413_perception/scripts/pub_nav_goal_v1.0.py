...
class FurthestBoxNavigator:
    ...
    #     # 1：Brige
    #     if self.bridge_found:
    #         if self.bridge_position and not self.last_bbox_msg and self.bridge_ready:
    ...
    #         if self.bridge_found and self.bridge_ready:
    #             rospy.loginfo("[Main Loop] All boxes matched, bridge is ready.")
    #             self.exit_fallback_mode()
    #             self.last_goal_position = self.bridge_position
    #             self.publish_goal(self.bridge_position)
    #             self.publish_arrow_marker(self.bridge_position)
    ...
    def find_fallback_goal(self):
        ...
        if box_count >= 10 and len(label_set) >= 4 and not self.bridge_found:
        ...
        elif box_count >= 10 and len(label_set) >= 4 and self.bridge_found:
            ...
            center_y = y_vals[int(np.clip(round(center_index[0]), 0, len(y_vals)-1))]
            center_x = x_vals[int(np.clip(round(center_index[1]), 0, len(x_vals)-1))]
    ...
    def bbox_callback(self, msg):
        ...
        self.try_publish_bridgehead_if_ready(msg, self.label_set, self.bridge_found)
        rospy.loginfo(f"[BBoxCallback] Bridge found: {self.bridge_found}, unmatched boxes: {unmatched_box_count}")
        rospy.loginfo(f"[BBoxCallback] Label set: {self.label_set}")
        ...
        if self.bridge_ready and not self.bridge_goal_sent and self.in_fallback_mode and self.bridge_position:
    ...
    def try_publish_bridgehead_if_ready(self, msg, label_set, bridge_found):
        ...
        if (not any_unmatched and box_count >= 10 and bridge_count == 1 and
            ...
            rospy.loginfo(f"[BridgeCheck] box_count={box_count}, bridge_count={bridge_count}, "
              ...
              f"bridge_position_x={(bridge_position.x if bridge_position else None)}")
            
            rospy.loginfo(f"[Navigator] All boxes matched and 4 labels found, bridge exists self.bridge_ready:{self.bridge_ready}.")
            self.bridge_ready = True
            self.bridge_position = [bridge_position.x + self.bridge_length/2 - 0.5 , bridge_position.y , bridge_position.z]

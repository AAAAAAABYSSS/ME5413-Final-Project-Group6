...
class PointCloudProcessor:
    ...
    def pointcloud_callback(self, msg):
        ...
        if (
            ...
            rospy.logwarn("检测由旋转导致的异常点云帧，已跳过保存")
            return
        ...
        if len(foreground_points) == 0:
            rospy.logwarn("没有检测到前景点，跳过保存")
            return

        # 进行聚类
        fg_pcd = o3d.geometry.PointCloud()
        fg_pcd.points = o3d.utility.Vector3dVector(foreground_points)
        # labels = np.array(fg_pcd.cluster_dbscan(eps=0.20, min_points=50))   # 桥后的识别不到
        labels = np.array(fg_pcd.cluster_dbscan(eps=0.50, min_points=30))
        ...
        rospy.loginfo(f"当前 all_detections 物体数量: {len(self.all_detections)}")
    ...
    def merge_bboxes(self, bboxes, threshold=0.05, max_iterations=5):
        ...
        for iteration in range(max_iterations):
            rospy.loginfo(f"第 {iteration + 1} 次迭代合并...")

            merged_bboxes = []
            merge_occurred = False
            remaining_bboxes = list(bboxes)  # 复制一份，避免修改原列表
            ...
            while remaining_bboxes:
                ...
                #     # if base_bbox["type"] == bbox["type"] and self.calculate_iou(base_bbox, bbox) > threshold:
                #     #     to_merge.append(bbox)
                #     #     merge_occurred = True
                #     # else:
                #     #     new_remaining.append(bbox)
                #     if base_bbox["type"] == bbox["type"]:
                #         if bbox["type"] == "box" and self.calculate_iou(base_bbox, bbox) > threshold:
                #             to_merge.append(bbox)
                #             merge_occurred = True
                ...
                remaining_bboxes = new_remaining  # 更新未合并的 bboxes

            if not merge_occurred:
                rospy.loginfo("没有新的合并发生，提前结束循环。")
                break

            bboxes = merged_bboxes  # 更新 bboxes 进行下一轮迭代
    ...
    def merge_two_bboxes(self, box1, box2):
        ...
        if not isinstance(box1, dict) or not isinstance(box2, dict):
            rospy.logwarn("尝试合并的 bbox 不是有效字典，已跳过")
            return box1  # 避免合并错误，直接返回原 bbox
    ...
    def publish_markers(self):
        ...
        for idx, detection in enumerate(self.all_detections):
            marker = Marker()
            marker.header.frame_id = "map"  # 或者你用的坐标系
            # marker.header.frame_id = "map"  # 或者你用的坐标系
            ...
            marker.lifetime = rospy.Duration(0.5)  # 显示时间，可调
